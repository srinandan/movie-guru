...
gcloud builds submit --config=deploy/register.yaml --region ${REGION} --worker-pool="projects/${PROJECT_ID}/locations/${REGION}/workerPools/movie-guru" --project=${PROJECT_ID} --async --ignore-file=.gcloudignore --substitutions=_PROJECT_ID="${PROJECT_ID}",\
_REGION="${REGION}"
